...
def main(nome_cartella):

    # ----------------------------------------
    # Preparation
    # ----------------------------------------

    noise_level_img = 10/255.0            # set AWGN noise level for LR image, default: 0, 
    ...
    testset_name = f'{nome_cartella}'             # set test set,  'set5' | 'srbsd68'
    x8 = True                            # default: False, x8 to boost performance
    test_sf = [1]                        # set scale factor, default: [2, 3, 4], [2], [3], [4]
    iter_num = 10                  # set number of iterations, default: 24 for SISR
    ...
    for sf in test_sf:
        ...
        k_num = 1 if classical_degradation else 1
...
if __name__ == '__main__':
    ...
    lista_cartelle_fatte = ['latent-diffusion_noise2image_FFHQ', 'biggan_512', 'guided-diffusion_noise2image_LSUNhorses', 'latent-diffusion_noise2image_LSUNbedrooms', 'stylegan2_ffhq_256x256', 'glide_text2img_valid', 'stylegan3_r_ffhqu_256x256', 'guided-diffusion_noise2image_LSUNcats', 'guided-diffusion_class2image_ImageNet', 'stylegan3_t_afhqv2_512x512', ".DS_Store"]

    for i in lista_cartelle:

        if i in lista_cartelle_fatte:
            pass
        else:
            '''
            if i == "stable_diffusion_256" or i == "dalle-mini_valid":
                main(i)
            '''
            if i == "dalle-mini_valid":
                main(i)
